...
public abstract class ExpenseCommandService {
    // 유저의 지출 내역 생성
    public abstract Expense createExpense(Long userId, ExpenseRequestDTO.ExpenseDTO request);

    // 유저의 지출 내역 수정
    public abstract Expense updateExpense(Long userId, Long expenseId, ExpenseRequestDTO.ExpenseUpdateDTO request);
